...
@Component
@Deprecated
public class ToutiaoIndexInfoCrawler extends BaseCrawler {
...
//    private static List<NewsContentArticleDO> ARTICLE_LIST = new ArrayList<>();
    ...
    private static String LIST_URL = "http://lf.snssdk.com/api/news/feed/v80/?fp=GSTqFM4ZJYwOFlP5F2U1FYmSFYGO&version_code=6.6.5" +
            ...
            "&aid=13&ab_version=304488,271329,297977,319439,317498,318244,295827,323881,239095,324283,170988,170989,325960,320218,325198,327237," +
            "281392,297059,314417,276203,286212,313219,328613,328678,277769,329942,322322,327535,326537,317410,328094,330205,328672,327416,324798," +
            "317077,324570,280773,330086,319957,326729,317210,321406,326344,301282,214069,320084,302116,318436,326692,258356,247850,280449,281299," +
            "328218,320995,325618,324093,328059,321414,288416,290197,260650,326192,327183,324614,271178,326587,326524,326532&ab_feature=z1" +
            "&openudid=68964f7bfc6391f424eeabb2c4eaa2a1c783148b&pos=5pe9vb%252F88Pzt0fLz%252BvTp6Pn4v72nvaysrrOkrqWopK2sq6WopaqvsZe9vb%252F88Pzt3vTp5L%" +
            "252B9p72%252FeyoseAEueCUfv7GXvb2%252F%252FPD87dH86fTp6Pn4v72nva%252Bvs6ior66kqqyurKirq6yvsZe9vb%252Fx%252FOn06ej5%252BL%252B9p72vr7OoqK%252" +
            "BupKqsrqyoq6usr7GXvb2%252F8fLz%252BvTp6Pn4v72nvaysrrOkrqWopK2sq6WopaqvsZe9vb%252F%252B9Onkv72nvb97Kix4AS6%252Fl%252BA%253D&idfv=16EC63C1-91" +
            "7A-448F-A9B7-CBC78DA014E9&ac=WIFI&os_version=10.3.3&ssmix=a&device_platform=iphone&iid=30058175430&ab_client=a1,f2,f7,e1&device_type=iPhone%206S" +
            "&idfa=F6F24C30-A099-42C1-A503-FF6F14440A49&LBS_status=authroize&city=%E6%B7%B1%E5%9C%B3&concern_id=6286225228934679042&count=20&cp=5aAfD7Da3876Bq1" +
            "&detail=1&image=1&language=zh-Hans-CN&last_refresh_sub_entrance_interval=1524447083&latitude=22.55239713156612&list_count=17&loc_mode=1" +
            "&loc_time=1524447068&longitude=113.9385901685872&min_behot_time=1524447071&refer=1&refresh_reason=2&session_refresh_idx=2&st_time=13&strict=0" +
            "&support_rn=4&tt_from=tab&as=a235139d1b769a675d6937&ts=1524447083 HTTP/1.1";

    // 初始化首页Cookie信息
    private Request setCookie(Request request) {
        request.addCookie("_ba", "BA0.2-20170301-51e32-7qosFmkl6jK8TG4cRnmr");
        request.addCookie("CNZZDATA1272189606", "1537112480-1523576421-%7C1524292076");
        ...
        request.addCookie("alert_coverage", "94");
...
//                    Integer share_count = (content.getInteger("share_count") != null ? content.getInteger("share_count") : 0);
                    ...
                    NewsDO newsDO =  newsCoreManager.buildNewsDO(newId, item_id.toString(), title, hot, display_url, share_url, source, article_url,
                            NewsDO.NEWS_TYPE_HEADLINE, NewsDO.CONTENT_TYPE_LARGE_PIC, keywords,ban_comment,
...
//            JSONArray ja = JSONArray.parseArray(JSON.toJSONString(list));
     ...
     * @param page
     */
    private void getNewsDetail(Page page) {
        try {
            JSONObject jsonObject = JSON.parseObject(page.getRawText());
