 ...
 */
@Component
public class YahooSportHomeCrawler extends BaseCrawler {

    private static final Logger LOG = LoggerFactory.getLogger(YahooSportHomeCrawler.class);
    private static final String URL = "https://sports.yahoo.com/";
    ...
    private void crawlerList(Page page) {
        LOG.info("抓取 Yahoo 体育之家新闻 begin!");
        Selectable sports = page.getHtml().xpath("//div[@id='Col1-3-SportsStream']");
        List<Selectable> nodes = sports.xpath("//ul[@class='Mb(0)']").$(".js-stream-content").nodes();

        if (CollectionUtil.isEmpty(nodes)) {
            LOG.error("Yahoo 体育之家 node 为空");
        }
        ...
        for (Selectable node : nodes) {
            ...
            String newsSource = NewsDO.DATA_SOURCE_YAHOO_SPORT_HOME;
