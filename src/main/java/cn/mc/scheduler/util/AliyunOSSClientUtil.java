...
public class AliyunOSSClientUtil {
    ...
    public String uploadPicture(String url) {
        try {
            ...
            uploadObject2OSS(ossClient, inputStream,
    ...
    public String replaceSourcePicToOSS(String content) {
        try {
            ...
            Selectable htmlNode = new Html(content);
            List<Selectable> imgNodes = htmlNode.xpath("//img").nodes();

            String regex = "(^//.*|^http.*)";
            boolean isImagePrefix = Pattern.matches(regex, content);
            if (isImagePrefix) {
                // html 和 图片
                ...
                for (Selectable imgNode : imgNodes) {
                    ...
                    if (isQRcode) {
                        ...
                        elements.attr("src", picUrl);
                    }
                }

                // 转换
                String articleHtml = htmlNode.xpath("//body/html()").toString();
                return articleHtml;
            }
        } catch (Exception ex) {
            LOGGER.error(ex.getMessage());
            throw new RuntimeException(ExceptionUtils.getMessage(ex));
        }
//        try{
...
//                        //查询图片是否是二维码
//                        Boolean isQRcode=checkImgQRcode(url);
//                        if(isQRcode){
//                            matcherForAttrib.appendReplacement(sbreplace, "");
//                        }else{
