 ...
 */
@Component
public class FengHuangSportNewsCrawler extends BaseCrawler {

    private Site site = Site.me().setRetryTimes(3).setSleepTime(2000);

    private static String URL = "https://isports.ifeng.com/tjwData/0/data.shtml?_=";

    private static String href = "http://ifenghuanghao.ifeng.com/";

    private static String html = "/news.shtml";
        ...
        if (initUrl.contains(page.getUrl().toString())) {
            ...
            if (!StringUtils.isEmpty(jsonReturn)) {
                ...
                for (int i = 0; i < dataJSONOArray.size(); i++) {
                    ...
                    //获取展示类型
                    Object type = jsonDataObject.get("type");
                    //如果没有type类型的过滤掉
                    ...
                    StringBuffer newsSourceUrl = new StringBuffer();
                    //如果是多图类型的
                    if (type.equals("article")) {
                        Object image = jsonDataObject.get("thumbnail");
...
//                        String dataKey= EncryptUtil.encrypt(newsSourceUrl.toString(), "md5");
                        ...
                        //获取wemediaEArticleId参数
                        Object wemediaId = jsonDataObject.get("wemediaEArticleId");
                        //如果没则丢弃
                        if (StringUtils.isEmpty(wemediaId)) {
                            continue;
                        }
                        newsSourceUrl.append(href).append(wemediaId.toString()).append(html);
                    } else {//先只抓图文其他类型的先不抓
                        continue;
                    }
                    String dataKey = EncryptUtil.encrypt(newsSourceUrl.toString(), "md5");
                    ...
                    NewsDO newsDO = newsCoreManager.buildNewsDO(
                            ...
                            newsUrl, shareUrl, null, newsSourceUrl.toString(),
